...
def get(cached=True):
    ...
    # ip_url = '''http://webapi.http.zhimacangku.com/getip?num=1&type=2&pro=&city=0&yys=0&port=11&time=1&ts=0&ys=0&cs=0&lb=1&sb=0&pb=45&mr=1&regions='''
    ...
    ip_url = '''http://http.tiqu.alicdns.com/getip3?num=1.txt&type=1.txt&pro=&city=0&yys=0&port=1.txt&time=1.txt&ts=0&ys=0&cs=0&lb=1.txt&sb=0&pb=4.txt&mr=1.txt&regions='''
    ...
    # ip_port = {'https': '%s' % ip_port, 'http': '%s' % ip_port}
    ip_port = {'https': '%s' % ip_port}
    ...
    #     s = re.sub('\D', "", json["msg"])
    #     time.sleep(int(s))
    #     return get_brand_product_id()
    # elif json["code"] == 116 or json["code"] == 115 or json["code"] == 121:
    #     # 套餐包已用完
    #     return get_brand_product_id(cached)
    # else:
...
# def get_proxy(cached=True):
    ...
    msg =ip_port['https']
    if '请添加白名单' in msg:
        dict_json= json.loads(msg)['msg']
        # print(dict_json.lstrip("请添加白名单"))
        ip = dict_json.lstrip("请添加白名单")
        print(ip)
        join_white_list(ip)
...
def join_white_list(ip):
    """加入白名单"""
    url = 'http://wapi.http.cnapi.cc/index/index/save_white?neek=38118&appkey=e0b0871f0c31983540dd42f6135a105a&white=' + ip
    print(url)
